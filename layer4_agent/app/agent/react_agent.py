...
class HoneypotAgent:
    ...
    def _is_safety_refusal(self, text: str) -> bool:
        return any(phrase in text.lower() for phrase in SAFETY_REFUSAL_PHRASES)
    ...
    def run_query(self, initial_prompt: str, system_instruction_override: str = None, max_turns: int = 5) -> str:
        state = AgentState()
        chosen_prompt = system_instruction_override if system_instruction_override else HONEYPOT_SYSTEM_PROMPT
        state.messages.append(SystemMessage(content=chosen_prompt))
        ...
        for turn in range(max_turns):
            ...
            if self._is_safety_refusal(generation):
                 return "Oh dear... I'm sorry, my screen just went completely blank. Tommy usually helps me with these things. Can you call back in 5 minutes?"
